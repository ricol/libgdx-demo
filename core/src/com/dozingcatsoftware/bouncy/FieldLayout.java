...
public class FieldLayout
{
    ...
    static void readLayoutArray()
    {
        try
        {
            InputStream fin = Gdx.files.internal("pinball/data/field_layout.json").read();
    ...
    public FieldLayout(Map layoutMap, World world)
    {
        ...
        this.ballColor = (layoutMap.containsKey("ballcolor")) ? (List<Integer>) layoutMap.get("ballcolor")
            : DEFAULT_BALL_COLOR;
    ...
    public List<Float> getLaunchVelocity()
    {
        ...
        if (launchMap.containsKey("random_velocity"))
        {
            ...
            if (delta.get(0).floatValue() > 0)
                vx += delta.get(0).floatValue() * RAND.nextFloat();
            if (delta.get(1).floatValue() > 0)
                vy += delta.get(1).floatValue() * RAND.nextFloat();
        }
    ...
    /**
     * Returns the desired ratio between real world time and simulation time. The application should adjust the frame rate and/or time interval passed to Field.tick() to keep the ratio as close to this value as possible.
     */
