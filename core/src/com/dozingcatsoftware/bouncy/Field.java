...
public class Field implements ContactListener
{
    ...
    /**
     * Creates Box2D world, reads layout definitions for the given level (currently only one), and initializes the game to the starting state.
     *
     * @param level
     */
...
// }
    ...
    /**
     * Called to advance the game's state by the specified number of milliseconds. iters is the number of times to call the Box2D World.step method; more iterations produce better accuracy. After updating physics, processes element collisions, calls tick() on every FieldElement, and performs scheduled actions.
     */
    ...
    /**
     * Called when a ball has ended. Ends the game if that was the last ball, otherwise updates GameState to the next ball. Shows a game message to indicate the ball number or game over.
     */
    ...
    /**
     * Returns true if there are active elements in motion. Returns false if there are no active elements, indicating that tick() can be called with larger time steps, less frequently, or not at all.
     */
    ...
    /**
     * Called to engage or disengage all flippers. If called with an argument of true, and all flippers were not previously engaged, calls the flipperActivated methods of all field elements and the field's delegate.
     */
    ...
    /**
     * Ends a game in progress by removing all balls in play, calling setGameInProgress(false) on the GameState, and setting a "Game Over" message for display by the score view.
     */
    public void endGame()
    {
        int len = balls.size();
