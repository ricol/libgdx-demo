...
public class Field implements ContactListener
{
	...
	static class ScheduledAction implements Comparable<ScheduledAction>
	{
		...
		{
			// sort by action time so these objects can be added to a PriorityQueue in the
			// right order
	...
	/**
	 * Creates Box2D world, reads layout definitions for the given level (currently
	 * only one), and initializes the game to the starting state.
	 * 
	 * @param level
	 */
	public void resetForLevel(int level)
	{
		...
		// map bodies and IDs to FieldElements, and get elements on whom tick() has to
		// be called
...
// }
	...
	/**
	 * Returns the FieldElement with the given value for its "id" attribute, or null
	...
	/**
	 * Called to advance the game's state by the specified number of milliseconds.
	 * iters is the number of times to call the Box2D World.step method; more
	 * iterations produce better accuracy. After updating physics, processes element
	 * collisions, calls tick() on every FieldElement, and performs scheduled
	 * actions.
	 */
	...
	/**
	 * Runs actions that were scheduled with scheduleAction and whose execution time
	 * has arrived.
	 */
	...
	/**
	 * Schedules an action to be run after the given interval in milliseconds has
	 * elapsed. Interval is in game time, not real time.
	 */
	...
	/**
	 * Launches a new ball. The position and velocity of the ball are controlled by
	 * the "launch" key in the field layout JSON.
	 */
	public Body launchBall()
	{
		...
		Body ball = Box2DFactory.createCircle(world, position.get(0).floatValue(), position.get(1).floatValue(), radius,
				false);
		ball.setBullet(true);
	...
	/**
	 * Removes a ball from play. If this results in no balls remaining on the field,
	 * calls doBallLost.
	 */
	 ...
	 * otherwise updates GameState to the next ball. Shows a game message to
	 * indicate the ball number or game over.
	 */
	public void doBallLost()
	{
		...
		if (hasExtraBall)
			...
			msg = "Ball " + this.gameState.getBallNumber();
	...
	/**
	 * Returns true if there are active elements in motion. Returns false if there
	 * are no active elements, indicating that tick() can be called with larger time
	 * steps, less frequently, or not at all.
	 */
	public boolean hasActiveElements()
	{
		// HACK: to allow flippers to drop properly at beginning of game, we need
		// accurate simulation
	...
	/**
	 * Removes balls that are not in play, currently defined as those having a y
	 * position of less than 1.
	 */
	...
	public void drawBalls(IFieldRenderer renderer)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			renderer.fillCircle(ball.getPosition().x, ball.getPosition().y, shape.getRadius(), color.get(0),
					color.get(1), color.get(2));
		}
	}

	/**
	 * Called to engage or disengage all flippers. If called with an argument of
	 * true, and all flippers were not previously engaged, calls the
	 * flipperActivated methods of all field elements and the field's delegate.
	 */
	public void setAllFlippersEngaged(boolean engaged)
	{
		...
		{
			if (allFlippersPreviouslyActive && !flipper.isFlipperEngaged())
	...
	/**
	 * Ends a game in progress by removing all balls in play, calling
	 * setGameInProgress(false) on the GameState, and setting a "Game Over" message
	 * for display by the score view.
	 */
	public void endGame()
	{
		int len = balls.size();
	...
	/**
	 * Called after Box2D world step method, to notify FieldElements that the ball
	 * collided with.
	 */
	...
	{
		// A ball can have multiple contacts (e.g. against two walls), so store list of
		// contacted fixtures
		Body ball = null;
	...
	/**
	 * Displays a message in the score view for the specified duration in
	 * milliseconds. Duration is in real world time, not simulated game time.
	 */
	...
	/**
	 * Adds the given value to the game score. The value is multiplied by the
	 * GameState's current multipler.
	 */
