...
/**
 * This FieldElement subclass represents a set of drop targets, which are
 * segments that disappear when hit. When all targets are hit, the Field
 * delegate is notified, and if the reset parameter is set, the targets will
...
public class DropTargetGroupElement extends FieldElement
{
	...
	// store all bodies and positions, use Body's active flag to determine which
	...
	public void finishCreate(Map params, World world)
	{
		...
		for (List pos : positions)
		{
			...
			{ asFloat(pos.get(0)), asFloat(pos.get(1)), asFloat(pos.get(2)), asFloat(pos.get(3)) };
	...
	/**
	 * Returns true if all targets have been hit (and their corresponding bodies
	 * made inactive)
	 */
