...
/**
 * FieldElement subclass that represents a straight wall. Its position is
 * specified by the "position" parameter with 4 values, which are [start x,
 * start y, end x, end y]. There are several optional parameters to customize
 * the wall's behavior: "kick": impulse to apply when a ball hits the wall, used
 * for kickers and ball savers. "kill": if true, the ball is lost when it hits
 * the wall. Used for invisible wall below the flippers. "retractWhenHit": if
 * true, the wall is removed when hit by a ball. Used for ball savers.
 * "disabled": if true, the wall starts out retracted, and will only be shown
 * when setRetracted(field, true) is called.
 * 
 * Walls can be removed from the field by calling setRetracted(field, true), and
 * restored with setRetracted(field, false).
 * 
 * @author brian
 */
...
public class WallElement extends FieldElement
{
	...
	{
		// tick() only needs to be called if this wall provides a kick which makes it
		// flash
	...
	Vector2 impulseForBall(Body ball)
	{
		...
		// dot product of (ball center - wall center) and impulse direction should be
		// positive, if not flip impulse
