...
/**
 * FieldElement subclass which represents a series of wall segments. The segments are defined in the "positions" parameter as a list of [x,y] values, for example: { "class": "WallPathElement", "positions": [[5,5], [5,10], [8,10], [5, 15]] }
 *
 * @author brian
 */
public class WallPathElement extends FieldElement
{
    ...
    public void finishCreate(Map params, World world)
    {
        ...
        for (int i = 0; i < positions.size() - 1; i++)
        {
            ...
            float[] segment = new float[]
            {
                asFloat(startpos.get(0)), asFloat(startpos.get(1)), asFloat(endpos.get(0)), asFloat(endpos.get(1))
            };
