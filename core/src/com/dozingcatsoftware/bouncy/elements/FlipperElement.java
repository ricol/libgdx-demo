...
/**
 * FieldElement subclass for a flipper that is controlled by the player. A
 * flipper consists of a Box2D RevoluteJoint where a thin wall rotates around an
 * invisible anchor. Flippers are defined in the layout JSON as follows: {
 * "class": "FlipperElement", "position": [5.5, 10], // x,y of fixed end of
 * flipper which it rotates around "length": 2.5, // length of the flipper.
 * Negative if the flipper rotates around its right end. "minangle": -20, //
 * minimum angle from the horizontal. Negative angles are below horizontal.
 * "maxangle": 20, // maximum angle from the horizontal. "upspeed": 7, // rate
 * at which the flipper rotates up when activated (in radians/sec?) "downspeed":
 * 3 // rate at which the flipper rotates down when not activated (in
 * radians/sec?) }
 * 
 * @author brian
 */
...
public class FlipperElement extends FieldElement
{
	...
	/**
	 * Returns the motor speed of the Box2D joint, normalized to be positive when
	 * the flipper is moving up.
	 */
	...
	public void tick(Field field)
	{
		...
		// if angle is at maximum, reduce speed so that the ball won't fly off when it
		// hits
	...
	{
		// only adjust speed if state is changing, so we don't accelerate flipper that's
		// been slowed down in tick()
	...
	public void draw(IFieldRenderer renderer)
	{
		...
		if (angle < jointDef.lowerAngle)
			angle = jointDef.lowerAngle;
		if (angle > jointDef.upperAngle)
			angle = jointDef.upperAngle;
		float x1 = position.x;
		float y1 = position.y;
