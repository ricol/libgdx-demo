...
public class RolloverGroupElement extends FieldElement
{
    ...
    /**
     * Cycles the states of all rollover elements by "rotating" right. For example, if this group has three rollovers whose states are (on, on, off), after calling this method the states will be (off, on, on). The state of the last rollover wraps around to the first, so (off, off, on) -> (on, off, off).
     */
