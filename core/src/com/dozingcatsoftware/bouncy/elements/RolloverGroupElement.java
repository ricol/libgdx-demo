...
/**
 * This class represents a collection of rollover elements, such as the
 * rollovers in the top lanes. They are activated (and optionally deactivated)
 * when a ball passes over them. Individual rollovers in the group are
 * represented by instances of the Rollover nested class, which specify center,
 * radius, and color. Parameters at the collection level control whether the
 * rollovers should cycle when flippers are activated, and whether rollovers can
...
public class RolloverGroupElement extends FieldElement
{
	...
	public void finishCreate(Map params, World world)
	{
		...
		for (Map rmap : rolloverMaps)
		{
			...
			// radius, color, score, and reset delay can be specified for each rollover, if
			// not present use default from group
	...
	/**
	 * Returns a set of all rollovers which have balls within their specified
	 * radius.
	 */
	...
	/**
	 * Activates the first unactivated rollover in the group. Has no effect if all
	 * are active.
	 */
	...
	public void tick(Field field)
	{
		...
		// only update rollovers that are hit on this tick and weren't on the previous
		// tick
		int len = hitRollovers.size();
		for (int i = 0; i < len; i++)
		{
			final Rollover rollover = hitRollovers.get(i);
			if (rolloversHitOnPreviousTick.contains(rollover))
				continue;
			// Inactive rollover becomes active, active rollover becomes inactive if
			// toggleOff setting is true.
	...
	/**
	 * Cycles the states of all rollover elements by "rotating" right. For example,
	 * if this group has three rollovers whose states are (on, on, off), after
	 * calling this method the states will be (off, on, on). The state of the last
	 * rollover wraps around to the first, so (off, off, on) -> (on, off, off).
	 */
	...
	/**
	 * Sets all rollovers to be active or inactive according to the boolean
	 * argument.
	 */
	...
	public void draw(IFieldRenderer renderer)
	{
		...
		// for each rollover, draw outlined circle for inactive or filled circle for
		// active#
		int len = rollovers.size();
		for (int i = 0; i < len; i++)
		{
