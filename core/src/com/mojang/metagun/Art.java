...
public class Art {
	...
	public static void load () {
		...
		level = new Pixmap(Gdx.files.internal("metagun/res/levels.png"));
		titleScreen = load("res/titlescreen.png", 320, 740);
		guys = split("res/guys.png", 6, 6);
		player1 = split("res/player.png", 16, 32);
		player2 = split("metagun/res/player.png", 16, 32, true);
	...
	private static TextureRegion[][] split (String name, int width, int height) {
		return split("metagun/" + name, width, height, false);
	}
	...
	public static TextureRegion load (String name, int width, int height) {
		Texture texture = new Texture(Gdx.files.internal("metagun/" + name));
